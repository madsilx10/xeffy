...
# ── Core logic per akun ──────────────────────────────────
async def run_account(session_string, index, tele_links, checkin_only=False):
    print(f"\n{'='*50}")
    ...
    async with Client(
        ...
        # Join Telegram channels/groups dari channel.txt
        if tele_links:
    ...
    if checkin_only:
        print(f"[Akun {index}] ✅ Selesai! (check-in only)")
        return
...
async def main():
    ...
    mode = input("\nMode (1. Full  2. Check-in only): ").strip()
    checkin_only = mode == "2"

    for i in indices:
        await run_account(sessions[i], i + 1, tele_links, checkin_only=checkin_only)
        if i != indices[-1]:
